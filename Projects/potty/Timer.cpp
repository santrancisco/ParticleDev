...
int8_t NewTimer::every(unsigned long period, void (*callback)(), int repeatCount)
{
...
int8_t NewTimer::every(unsigned long period, void (*callback)())
{
	return every(period, callback, -1); // - means forever
}

int8_t NewTimer::after(unsigned long period, void (*callback)())
{
	return every(period, callback, 1);
}

int8_t NewTimer::oscillate(uint8_t pin, unsigned long period, uint8_t startingValue, int repeatCount)
{
	int8_t i = findFreeEventIndex();
...
int8_t NewTimer::oscillate(uint8_t pin, unsigned long period, uint8_t startingValue)
{
 ...
 */
int8_t NewTimer::pulse(uint8_t pin, unsigned long period, uint8_t startingValue)
{
 ...
 */
int8_t NewTimer::pulseImmediate(uint8_t pin, unsigned long period, uint8_t pulseValue)
{
...
int8_t NewTimer::findFreeEventIndex(void)
{
	for (int8_t i = 0; i < MAX_NUMBER_OF_EVENTS; i++)
	{
		if (_events[i].eventType == EVENT_NONE)
